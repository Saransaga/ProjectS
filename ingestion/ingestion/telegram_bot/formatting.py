...
from ..recommendation.price_levels import pace_estimate_days, resolve_price_targets
...
def _format_pace(close: float, target_price: float, atr_14: float | None) -> str | None:
    """Renders recommendation/price_levels.py::pace_estimate_days into the
    bot's text line — deliberately framed as a pace, not a prediction, and
    never a fabricated calendar date."""
    days = pace_estimate_days(close, target_price, atr_14)
    if days is None:
        return None
...
def _format_price_level(action: str | None, levels: dict | None) -> list[str]:
    """Direction-aware target/exit lines, built from
    recommendation/price_levels.py::resolve_price_targets(). For a bullish
    action the target is above close (upside) and the exit trigger is below
    (where the bullish case breaks); a bearish action flips the two. Empty
    list for HOLD (no directional call to hang a target/exit on) or when
    ...
    target, guard = resolved["target"], resolved["stop"]

    target_label = "Target (resistance)" if bullish else "Downside target (support)"
    if target and target.get("projected"):
    ...
    if guard and guard.get("projected"):
    ...
    if target:
        ...
        if close is not None:
            pace = _format_pace(close, target["price"], levels.get("atr_14"))
            if pace:
...
def format_help() -> str:
    return (
        ...
        "/52wlow — stocks currently trending near their 52-week low\n"
        "/dividends — highest dividend-yield stocks"
    )
...
def format_52_week_lows(entries: list[dict], near_pct: float) -> str:
    """entries: query/snapshot.stocks_near_52_week_low's output."""
    if not entries:
        return f"No stocks currently within {near_pct:g}% of their 52-week low."
    lines = [f"Stocks near their 52-week low (within {near_pct:g}%) — {entries[0]['trade_date']}", ""]
    for rank, e in enumerate(entries, start=1):
        lines.append(
            f"{rank}. *{_esc(e['symbol'])}* — {_esc(e['name'])}\n"
            f"   Close {e['close']:.2f} vs 52w low {e['low_52w']:.2f} (+{e['pct_above_low']:.1f}%)"
        )
    return "\n".join(lines)


def format_dividend_leaders(entries: list[dict]) -> str:
    """entries: query/snapshot.top_dividend_yield's output."""
    if not entries:
        return "No dividend-yield data on record yet."
    lines = ["Highest dividend-yield stocks (trailing 12-month):", ""]
    for rank, e in enumerate(entries, start=1):
        lines.append(
            f"{rank}. *{_esc(e['symbol'])}* — {_esc(e['name'])}: "
            f"{e['dividend_yield']:.2f}% (as of {e['as_of_date']})"
        )
    return "\n".join(lines)
